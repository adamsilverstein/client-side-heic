 ...
 * @return bool
 */
function csh_is_heic_enabled() {
	return (bool) get_option( 'csh_heic_enabled', 1 );
}
 ...
 */
function csh_add_heic_mime_types( $mime_types ) {
...
add_filter( 'upload_mimes', 'csh_add_heic_mime_types' );
...
add_filter( 'wp_check_filetype_and_ext', 'csh_heic_check_filetype', 10, 5 );
 ...
 */
function csh_add_heic_client_side_mime_types( $mime_types ) {
	if ( ! csh_is_heic_enabled() ) {
		return $mime_types;
	}
...
add_filter( 'client_side_supported_mime_types', 'csh_add_heic_client_side_mime_types' );
 ...
 */
function csh_heic_output_format( $formats ) {
	if ( ! csh_is_heic_enabled() ) {
		return $formats;
	}
...
add_filter( 'image_editor_output_format', 'csh_heic_output_format' );
 ...
 */
function csh_enqueue_heic_scripts( $hook_suffix ) {
	if ( ! csh_is_heic_enabled() ) {
		return;
	}
	...
	$cdn_url = apply_filters(
		'csh_heic_library_url',
		'https://cdn.jsdelivr.net/npm/heic2any@0.0.4/dist/heic2any.min.js'
	);

	wp_enqueue_script(
		'csh-heic-support',
		CSH_PLUGIN_URL . 'js/heic-support.js',
	 ...
	 */
	$jpeg_quality = apply_filters( 'csh_heic_jpeg_quality', 0.92 );
...
add_action( 'admin_enqueue_scripts', 'csh_enqueue_heic_scripts' );
