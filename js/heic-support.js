...
/* global wp, cshHeicSupport */

( function () {
	'use strict';

	if ( ! window.cshHeicSupport || ! window.cshHeicSupport.cdnUrl ) {
		return;
	}

	var CDN_URL = window.cshHeicSupport.cdnUrl;
	var CDN_INTEGRITY = window.cshHeicSupport.cdnIntegrity || '';
	var JPEG_QUALITY =
		parseFloat( window.cshHeicSupport.jpegQuality ) || 0.92;
	var heic2anyPromise = null;
	var NOTICE_ID = 'csh-heic-converting';
				...
				reject(
					new Error(
						'Failed to load HEIC conversion library from CDN. ' +
							...
							'the csh_heic_library_url filter.'
					)
				);
			};
	...
	function wrapAddItems() {
		...
		if ( uploadStore.addItems.__cshHeicWrapped ) {
			return true;
		}
		...
		uploadStore.addItems.__cshHeicWrapped = true;
		return true;
	}
