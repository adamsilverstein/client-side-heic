 ...
 * @return bool
 */
function csh_should_use_coep_coop() {
	$chrome_version = null;
	 ...
	 */
	return (bool) apply_filters( 'csh_use_coep_coop', ! $use_dip );
}
 ...
 */
function csh_set_up_cross_origin_isolation() {
	if ( ! csh_should_use_coep_coop() ) {
		return;
	}
	...
	csh_start_coep_coop_output_buffer();
}

add_action( 'load-post.php', 'csh_set_up_cross_origin_isolation', 20 );
add_action( 'load-post-new.php', 'csh_set_up_cross_origin_isolation', 20 );
add_action( 'load-site-editor.php', 'csh_set_up_cross_origin_isolation', 20 );
add_action( 'load-widgets.php', 'csh_set_up_cross_origin_isolation', 20 );
 ...
 */
function csh_start_coep_coop_output_buffer() {
	global $is_safari;
...
add_action( 'admin_init', 'csh_set_js_flag' );
 ...
 */
function csh_enqueue_coep_scripts( $hook_suffix ) {
	if ( ! csh_should_use_coep_coop() ) {
		return;
	}
	...
	wp_enqueue_script(
		'csh-cross-origin-isolation-coep',
		CSH_PLUGIN_URL . 'js/cross-origin-isolation-coep.js',
...
add_action( 'admin_enqueue_scripts', 'csh_enqueue_coep_scripts' );
